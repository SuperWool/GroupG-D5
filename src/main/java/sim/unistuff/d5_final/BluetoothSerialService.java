 ...
 * Adapted from the BlueTerm app by Pymasde (pymasde.es, source code: https://code.google.com/p/
 * bluetooth-remote-control/source/browse/trunk/src/pro/apus/blueremote/BluetoothSerialService.java?r=3).
...
public class BluetoothSerialService {
	...
	private void connectionLost() {
		...
		Message msg = mHandler.obtainMessage(MainActivity.MESSAGE_TOAST);
		Bundle bundle = new Bundle();
		bundle.putString(MainActivity.TOAST, "Connection was lost");
		msg.setData(bundle);
		mHandler.sendMessage(msg);
	}
		...
		public void run() {
			...
			while (true) {
				try {
					...
					mHandler.obtainMessage(MainActivity.MESSAGE_READ, bytes, -1, message).sendToTarget();
				} catch (IOException e) {
